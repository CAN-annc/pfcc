...
export function calcGoldAssets(goldHoldings, goldPriceUsdOz, fxRates) {
  ...
  const enriched = (goldHoldings ?? []).map(g => {
    const grams = goldToGrams(g.weight, g.unit);
    const valueTWD = pricePerGramTwd != null ? grams * pricePerGramTwd : null;
    if (valueTWD != null) totalTWD += valueTWD;
...
export function calcNetWorth({ cashTWD, depositTWD, investmentTWD, otherAssetsTWD = 0, goldTWD = 0 }) {
  return cashTWD + depositTWD + investmentTWD + otherAssetsTWD + goldTWD;
}
...
/**
 * How many days AGO a date is, relative to today (positive = in the past,
 * 0 = today, negative = in the future). Just -daysUntil(), given its own
 * name because call sites reasoning about backdated entries (§廿六) read
 * more naturally in terms of "how long ago" than "days until".
 */
export function daysAgo(dateString) {
  if (!dateString) return 0;
  const u = daysUntil(dateString);
  return u == null ? 0 : -u;
}

/**
 * Default state for the 收入／支出 modal's "是否影響現有帳戶餘額" toggle
 * when backdating an entry (§廿六，2026-09-26 使用者透過 AskUserQuestion
 * 確認 3 天門檻). Today or a future date is never "backdating" — the caller
 * hides the toggle entirely in that case and this always affects. A past
 * date within `thresholdDays` still defaults to affecting the account (the
 * balance probably hasn't been reconciled since); further back defaults to
 * NOT affecting (the balance was probably already corrected some other way
 * by now, so applying it again would double-count).
 */
export function defaultAffectsBalance(dateString, thresholdDays = 3) {
  const ago = daysAgo(dateString);
  if (ago <= 0) return true;
  return ago <= thresholdDays;
}
